...
spark.sql(f"CREATE EXTERNAL TABLE IF NOT EXISTS demo.featureLoanTrx USING DELTA LOCATION '{gold_folder_path}/featureLoanTrx'")
...
# We create a table in our demo database so we can query it later
spark.sql(f"CREATE EXTERNAL TABLE IF NOT EXISTS demo.aggLoanTrx USING DELTA LOCATION '{gold_folder_path}/aggLoanTrx'")
